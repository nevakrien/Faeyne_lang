...
fn test_end_to_end_empty_function() {
    ...
    // Step 2: Setup the StringTable (we will use Arc<RwLock<StringTable>> as required)
    let string_table = Arc::new(RwLock::new(StringTable::new()));

    // Step 3: Parse the source code into an AST
    ...
    // Step 4: Translate the AST into the VM's bytecode using translate_program
    let code = match translate_program(&parsed_ast, string_table.clone()) {
        Ok(code) => code,
        Err(e) => {
    ...
    // Step 5: Setup the initial arguments for the "main" function (arbitrary values)
    let args = vec![
    ...
    // Step 6: Run the translated code and call the "main" function with the arguments
